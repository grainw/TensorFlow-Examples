...
# data I/O
data = open('../data/data.txt', 'rb').read() # should be simple plain text file
...
def lossFun(inputs, targets, hprev):
    ...
    for t in range(len(inputs)):
        ...
        xs[t][inputs[t]] = 1    # x[t] 是一个第t个输入单词的向量 shape为(size,1)
        ...
        loss += -np.log(ps[t][targets[t],0]) # softmax (cross-entropy loss) 代价函数是交叉熵 #正确的y乘以log预测的y

    # 将输入循环一遍以后，得到各个时间段的h, y 和 p
    # 得到此时累积的loss, 准备进行更新矩阵
    # backward pass: compute gradients going backwards
    dWxh, dWhh, dWhy = np.zeros_like(Wxh), np.zeros_like(Whh), np.zeros_like(Why) # 各矩阵的参数进行  对梯度的理解还是不够，晚上看邹博。
    ...
    for t in reversed(range(len(inputs))): # 把时间作为维度，则梯度的计算应该沿着时间回溯
        ...
        dby += dy # 这个没什么可说的，与dWhy一样，只不过h项=1， 所以直接等于dy  #标准输出减去实际输出 乘以 输入的转置 ，b偏移项的输入等于1.
        ####################################################
        dh = np.dot(Why.T, dy) + dhnext # backprop into h  z_t = Why*H_t + b_y H_t = tanh(Whh*H_t-1 + Whx*X_t), 第一阶段求导 #德尔塔前半部分
        dhraw = (1 - hs[t] * hs[t]) * dh # backprop through tanh nonlinearity  第二阶段求导，注意tanh的求导   #德尔塔后半部分双曲正切函数求导
        dbh += dhraw   # dbh表示传递 到h层的误差
        ##################################################### 对隐藏层的求导
        dWxh += np.dot(dhraw, xs[t].T)    # 对Wxh的修正，同Why
        dWhh += np.dot(dhraw, hs[t-1].T)  # 对Whh的修正
        dhnext = np.dot(Whh.T, dhraw)     # h层的误差通过Whh不停地累积  #总结起来就是德尔塔学习法则
    for dparam in [dWxh, dWhh, dWhy, dbh, dby]:
        np.clip(dparam, -5, 5, out=dparam) # clip to mitigate exploding gradients #修减参数在某个范围之内
